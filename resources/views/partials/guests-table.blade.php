        ...
        <tbody class="divide-y divide-slate-100 bg-white">
            @foreach($recentGuests as $guest)
                <tr class="hover:bg-blue-50/50 transition duration-150 group">
                    <td class="px-6 py-4">
                        <div class="font-bold text-slate-700 group-hover:text-blue-700 transition">{{ $guest->nama_tamu }}</div>
                        <div class="text-[10px] text-slate-400 font-medium">{{ $guest->jumlah_personil }} Personil</div>
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-block bg-slate-100 text-slate-600 text-xs px-2 py-1 rounded border border-slate-200">
                            {{ $guest->asal_instansi }}
                        </span>
                    </td>
...
<div class="px-6 py-3 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex justify-between items-center">
    <span>Menampilkan <strong>{{ count($recentGuests) }}</strong> data</span>
</div>