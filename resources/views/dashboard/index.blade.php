...
@section('content')
    ...
    <div class="min-h-screen bg-slate-50/50 py-8 relative">
        <div
            class="absolute top-0 left-0 w-full h-64 bg-gradient-to-b from-blue-600 to-blue-800 -z-10 rounded-b-[3rem] shadow-2xl">
        </div>
        ...
        <div class="container mx-auto px-4 md:px-8 relative z-10">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-12 text-white">
                ...
                <form action="{{ route('logout') }}" method="POST" class="mt-4 md:mt-0">
                    @csrf
                    <button type="submit"
                        class="bg-white/10 hover:bg-white/20 text-white px-6 py-3 rounded-xl font-semibold transition backdrop-blur-sm border border-white/20 flex items-center gap-2 group">
            ...
            @if(session('success'))
                <div
                    class="bg-green-500 text-white px-6 py-4 rounded-2xl mb-8 shadow-lg shadow-green-500/20 font-medium flex items-center gap-3 animate-fade-in-down">
            ...
            @if(session('error'))
                <div
                    class="bg-red-500 text-white px-6 py-4 rounded-2xl mb-8 shadow-lg shadow-red-500/20 font-medium flex items-center gap-3 animate-fade-in-down">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @if(Auth::user()->role == 'superadmin')
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <a href="{{ route('users.index') }}"
                        class="group bg-white p-8 rounded-3xl shadow-xl hover:shadow-2xl transition duration-300 border border-slate-100 relative overflow-hidden">
                        <div
                            class="absolute right-0 top-0 w-32 h-32 bg-red-50 rounded-bl-full -mr-8 -mt-8 transition group-hover:scale-110">
                        </div>
                        <div class="relative z-10">
                            <div
                                class="w-14 h-14 bg-red-100 text-red-600 rounded-2xl flex items-center justify-center mb-6 shadow-sm">
                            ...
                            <p class="text-slate-500 mt-2 text-sm leading-relaxed">Manajemen akun petugas dan hak akses sistem.
                            </p>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('absen.rekap') }}"
                        class="group bg-white p-8 rounded-3xl shadow-xl hover:shadow-2xl transition duration-300 border border-slate-100 relative overflow-hidden">
                        <div
                            class="absolute right-0 top-0 w-32 h-32 bg-purple-50 rounded-bl-full -mr-8 -mt-8 transition group-hover:scale-110">
                        </div>
                        <div class="relative z-10">
                            <div
                                class="w-14 h-14 bg-purple-100 text-purple-600 rounded-2xl flex items-center justify-center mb-6 shadow-sm">
                                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <p class="text-slate-500 mt-2 text-sm leading-relaxed">Rekapitulasi kehadiran harian seluruh
                                pegawai.</p>
                        </div>
                    </a>
                    ...
                    <a href="{{ route('guest.rekap') }}"
                        class="group bg-white p-8 rounded-3xl shadow-xl hover:shadow-2xl transition duration-300 border border-slate-100 relative overflow-hidden">
                        <div
                            class="absolute right-0 top-0 w-32 h-32 bg-blue-50 rounded-bl-full -mr-8 -mt-8 transition group-hover:scale-110">
                        </div>
                        <div class="relative z-10">
                            <div
                                class="w-14 h-14 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center mb-6 shadow-sm">
                                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div class="bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden relative">
                            ...
                            <div class="p-6">
                                <div class="text-center mb-6">
                                    <div id="digital-clock" class="text-5xl font-black text-slate-800 tracking-tight font-mono">
                                        --:--</div>
                                    <span class="text-sm text-slate-400 font-medium">Waktu Indonesia Tengah (WITA)</span>
                                </div>

                                <div
                                    class="relative rounded-2xl overflow-hidden shadow-inner border border-slate-200 mb-6 group">
                                    <div id="map" class="w-full h-48 z-0"></div>
                                    <div class="absolute top-2 right-2 z-[400]">
                                        <div id="gps-indicator"
                                            class="bg-white/90 backdrop-blur text-[10px] font-bold px-2 py-1 rounded-lg shadow-sm text-slate-600 flex items-center gap-1">
                                ...
                                <div id="location-status" class="mb-4">
                                    <div
                                        class="animate-pulse flex flex-col items-center justify-center p-4 bg-slate-50 rounded-xl">
                                ...
                                @if(!$attendance)
                                    <form action="{{ route('absen.masuk') }}" method="POST">
                                        ...
                                        <button type="submit" id="btn-absen" disabled
                                            class="w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all transform active:scale-95 flex items-center justify-center gap-2 bg-slate-300 cursor-not-allowed">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="lg:col-span-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <a href="{{ route('guest.rekap') }}"
                                class="group relative bg-white rounded-xl p-8 shadow-xl hover:shadow-2xl transition duration-300 border border-slate-100 overflow-hidden flex flex-col justify-between h-64">
                                <div
                                    class="absolute right-0 bottom-0 opacity-5 transform translate-y-4 translate-x-4 group-hover:scale-110 transition duration-500">
                                    <svg class="w-40 h-40 text-blue-900" fill="currentColor" viewBox="0 0 24 24">
                                        <path
                                ...
                                <div class="relative z-10">
                                    <div
                                        class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-blue-600 mb-4 group-hover:bg-blue-600 group-hover:text-white transition">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z">
                                            </path>
                                ...
                                <div
                                    class="absolute bottom-6 right-8 opacity-0 group-hover:opacity-100 transition transform translate-x-4 group-hover:translate-x-0">
                            ...
                            <a href="{{ route('absen.rekap') }}"
                                ...
                                <div
                                    class="absolute right-0 bottom-0 opacity-5 transform translate-y-4 translate-x-4 group-hover:scale-110 transition duration-500">
                                    <svg class="w-40 h-40 text-purple-900" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="relative z-10">
                                    <div
                                        class="w-12 h-12 bg-purple-50 rounded-lg flex items-center justify-center text-purple-600 mb-4 group-hover:bg-purple-600 group-hover:text-white transition">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2">
                                            </path>
                                ...
                                <div class="relative z-10 mt-auto">
                                    <span
                                        class="inline-flex items-center justify-center px-4 py-2 bg-purple-50 text-purple-700 font-bold text-sm rounded-xl group-hover:bg-purple-600 group-hover:text-white transition">
                                        Buka Laporan
                                    </span>
                                </div>
                            </a>
                        ...
                        <div class="mt-6 bg-blue-900 rounded-xl p-8 text-white relative overflow-hidden shadow-xl">
                            <div
                                class="absolute top-0 right-0 w-64 h-64 bg-white opacity-5 rounded-full -mr-16 -mt-16 blur-3xl">
                            </div>
                            <div class="relative z-10 flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-blue-200 text-sm leading-relaxed">
                                        Pastikan Anda berada dalam radius <strong>200 meter</strong> dari titik kantor saat
                                        melakukan absensi.
                                        Jika tombol absen tidak aktif, coba tekan tombol <strong>Refresh Lokasi</strong> atau
